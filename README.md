...
4. 点击「开始扫描」— 多线程递归扫描，对比出未注册文件
5. 结果表格：勾选框 | 文件名 | 完整路径 | 文件大小，支持排序
6. 可「全选」「取消全选」，点击「触发视频变动」— 逐个移出→等10s→移回，触发 VS 自动索引

#### 3.7 VS 追加剧集 meta 补全

**纯本地操作**，无需 DSM 配置。用于连载剧集后续追加的视频缺少 vsmeta 或 vsmeta 信息不全的场景。

**步骤**：
1. 在主界面选择目标文件夹（如某剧集的 Season 目录）
2. 点击「VS追加剧集meta补全」打开弹窗
3. 目标文件夹和中转文件夹均支持历史记录，通过「...」按钮浏览选择
4. 两者必须在同一一级目录下（如都在 `N:\video\` 下），否则弹窗提示
5. 点击「开始扫描」— 深度优先遍历，每个目录显示进度，自动找同目录同 season 的基准 vsmeta（最低 episode，跳过第 0 集）
6. 结果表格展示待补全的文件路径和具体缺失字段（如「剧集标题, 演员, 类型 缺失」）
7. 可「全选」「取消全选」，点击「确认补全」— 逐个从基准 vsmeta 复制元数据补全，再移出→等10s→移回触发 VS 刷新

#### 3.8 VS 剧集缩略图重刷

**纯本地操作**，无需 DSM 配置。用于重新生成剧集的视频截图缩略图。

**扫描模式**：
- **仅无缩略图**：只扫描 `episodeThumbData` 为空的 vsmeta
- **重复缩略图**：扫描同目录下多个 vsmeta 共享相同 MD5 的（批量补全时模板复制的残留）
- **无+重复**：上述两种取并集
- **全部重刷**：所有有效剧集 vsmeta 都重新生成

**步骤**：
1. 在主界面选择目标文件夹
2. 点击「VS剧集缩略图重刷」打开弹窗，选择扫描模式
3. 点击「开始扫描」— 深度优先遍历，加载 vsmeta 时显示进度，每目录即时筛选
4. 结果表格展示符合条件的 vsmeta，可「全选」「取消全选」
5. 点击「确认重刷」— 用 FFmpeg 在视频 61.8% 位置截取一帧，Base64 编码写入 vsmeta，再移出→等10s→移回触发 VS 刷新
